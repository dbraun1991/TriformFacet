...
- **Wedge cylinder gets a small, flat per-material emissive tint** (a
  scaled average of the three active light colors, applied in
  `styleManager.applyStyle()`) as a minor fill for the one sliver that's
  genuinely unlit by design (the bottom cut face — no light points that
  way). `render_scene.py`'s own object-color comment says the near-white
  material is chosen "so it visibly reflects each surface's light color,"
  and the *actual* mechanism behind that in three.js turned out to be a
  real bug fix, not this tint — see ADR 0034's inverted-winding finding.
  (An earlier pass, before that bug was found, misattributed the taper's
  flat-black appearance to Lambertian grazing-incidence falloff and used
  this tint at a much higher weight as the primary fix; also tried and
  abandoned, for the reason below, an object-only fill light via three.js
  `Layers`.) `Layers` genuinely doesn't work the way it sounds for this:
  light/object layer matching in three.js's forward renderer only gates a
  light against the *camera's* layers (whether the light is used in the
  render at all), not against which objects it illuminates once active —
  confirmed by testing it live (enabling the camera's layer washed the
  whole room, not just the object).
